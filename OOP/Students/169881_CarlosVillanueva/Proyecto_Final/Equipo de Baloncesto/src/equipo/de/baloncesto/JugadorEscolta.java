...
public class JugadorEscolta extends Jugador implements Jugadas {/*Se crea la clase JugadorEscolta, la cual hereda atributos
   de la clase padre Jugador y métodos de la interfaz Jugadas.*/
    
   public JugadorEscolta(String Nombre, String Posicion, boolean Visitante, int Estatura, int NivelPase, int NivelTiro){
   ...
   @Override
    ...
    public void Pasar(){//El método Pasar hace que el jugador pase el balón hacia uno de sus compañeros.
        ...
        this.setObjetivo((int) (Math.random()*5+6));/*Este número aleatorio se castea como entero porque el método Math.random genera números que incluyen
          decimales.*/
        }while(Jugador.getObjetivo()==6);/*Se repite esta acción hasta que el objetivo sea diferente al
        número 6 (el número que identifica a este jugador) para evitar que el jugador se pase el balón
        a él mismo.*/
        }
    ...
    public void Jugar(){/*El método Jugar hace que el jugador tome una desición en cuanto a lo que
        ...
        /*Almacena el texto indicado en el atributo ResumenDeJugadas de la clase Partido. Este texto indica 
        que el jugador ha conseguido el balón.*/
        do{
